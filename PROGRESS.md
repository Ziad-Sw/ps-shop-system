...
**تحديث:** تم عكس اتجاهات الشريط العلوي (e093d42):
...
تمت إضافة نقطة نهاية `POST /api/auth/logout` التي:
- تمسح كوكي الجلسة `ps_shop_session` عن طريق تعيينه بقيمة فارغة و`Max-Age=0`.
- توجه المستخدم إلى `/login` بعد الخروج.

كما أضيف زر "تسجيل الخروج" في `app/page.tsx` يظهر فقط عندما يكون المستخدم مسجلاً.
